...
type AlertCode = "A1" | "A2" | "A3" | "A4" | "A5" | "SF1" | "SF2";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // ── SF1: Scadenza 30 giugno (saldo + 1° acconto IRPEF/INPS) ──
    const now = new Date();
    const currentYear = now.getFullYear();

    const giugno30 = new Date(currentYear, 5, 30); // 30 giugno
    const novembre30 = new Date(currentYear, 10, 30); // 30 novembre

    const daysToGiugno   = Math.ceil((giugno30.getTime()   - now.getTime()) / 86400000);
    const daysToNovembre = Math.ceil((novembre30.getTime() - now.getTime()) / 86400000);

    const sf1Triggered = daysToGiugno >= 0 && daysToGiugno <= 7;
    results.push({
      code: "SF1",
      title: "Scadenza fiscale: 30 giugno",
      status: sf1Triggered ? "active" : "resolved",
      triggered: sf1Triggered,
      trigger_value: daysToGiugno,
      threshold: 7,
      message: daysToGiugno === 0
        ? "⚠️ Oggi scade il versamento saldo + 1° acconto IRPEF/INPS (F24). Verifica di avere liquidità sufficiente."
        : `Mancano ${daysToGiugno} giorni al 30 giugno — saldo + 1° acconto IRPEF/INPS (F24).`,
      metadata: { days_remaining: daysToGiugno, deadline: `${currentYear}-06-30` },
    });

    // ── SF2: Scadenza 30 novembre (2° acconto IRPEF/INPS) ────────
    const sf2Triggered = daysToNovembre >= 0 && daysToNovembre <= 7;
    results.push({
      code: "SF2",
      title: "Scadenza fiscale: 30 novembre",
      status: sf2Triggered ? "active" : "resolved",
      triggered: sf2Triggered,
      trigger_value: daysToNovembre,
      threshold: 7,
      message: daysToNovembre === 0
        ? "⚠️ Oggi scade il versamento 2° acconto IRPEF/INPS (F24). Verifica di avere liquidità sufficiente."
        : `Mancano ${daysToNovembre} giorni al 30 novembre — 2° acconto IRPEF/INPS (F24).`,
      metadata: { days_remaining: daysToNovembre, deadline: `${currentYear}-11-30` },
    });
